    ...
    AngularMeteorCollection.save = function save(docs, useUnsetModifier) {                                     // 18
      ...
      function upsertObject(item, $q) {                                                                        // 28
        var deferred = $q.defer();                                                                             // 29
                                                                                                               // 30
        if (item._id) { // Performs an update if the _id property is set.                                      // 31
          var item_id = item._id; // Store the _id in temporary variable                                       // 32
          delete item._id; // Remove the _id property so that it can be $set using update.                     // 33
          var objectId = (item_id._str) ? new Meteor.Collection.ObjectID(item_id._str) : item_id;              // 34
          var modifier = (useUnsetModifier) ? {$unset: item} : {$set: item};                                   // 35
                                                                                                               // 36
          collection.update(objectId, modifier, function (error) {                                             // 37
            if (error) {                                                                                       // 38
              deferred.reject(error);                                                                          // 39
            } else {                                                                                           // 40
              deferred.resolve({_id: objectId, action: "updated"});                                            // 41
            }                                                                                                  // 42
          });                                                                                                  // 43
        } else { // Performs an insert if the _id property isn't set.                                          // 44
          collection.insert(item, function (error, result) {                                                   // 45
            if (error) {                                                                                       // 46
              deferred.reject(error);                                                                          // 47
            } else {                                                                                           // 48
              deferred.resolve({_id: result, action: "inserted"});                                             // 49
            }                                                                                                  // 50
          });                                                                                                  // 51
        }                                                                                                      // 52
                                                                                                               // 53
        return deferred.promise;                                                                               // 54
      }                                                                                                        // 55
                                                                                                               // 56
      /*                                                                                                       // 57
       * How to update the collection depending on the 'docs' argument passed.                                 // 58
       */                                                                                                      // 59
      if (docs) { // Checks if a 'docs' argument was passed.                                                   // 60
        if (angular.isArray(docs)) { // If an array of objects were passed.                                    // 61
          angular.forEach(docs, function (doc) {                                                               // 62
            this.push(upsertObject(doc, $q));                                                                  // 63
          }, promises);                                                                                        // 64
        } else { // If a single object was passed.                                                             // 65
          promises.push(upsertObject(docs, $q));                                                               // 66
        }                                                                                                      // 67
      } else { // If no 'docs' argument was passed, save the entire collection.                                // 68
        angular.forEach(self, function (doc) {                                                                 // 69
          this.push(upsertObject(doc, $q));                                                                    // 70
        }, promises);                                                                                          // 71
      }                                                                                                        // 72
                                                                                                               // 73
      return $q.all(promises); // Returns all promises when they're resolved.                                  // 74
    };                                                                                                         // 75
                                                                                                               // 76
    AngularMeteorCollection.remove = function remove(keys) {                                                   // 77
      var self = this,                                                                                         // 78
        collection = self.$$collection,                                                                        // 79
        promises = []; // To store all promises.                                                               // 80
                                                                                                               // 81
      /*                                                                                                       // 82
       * The removeObject function will delete an object with the _id property                                 // 83
       * equal to the specified key.                                                                           // 84
       * Returns a promise.                                                                                    // 85
       */                                                                                                      // 86
      function removeObject(key, $q) {                                                                         // 87
        var deferred = $q.defer();                                                                             // 88
                                                                                                               // 89
        if (key) { // Checks if 'key' argument is set.                                                         // 90
          if (key._id) {                                                                                       // 91
            key = key._id;                                                                                     // 92
          }                                                                                                    // 93
          var objectId = (key._str) ? new Meteor.Collection.ObjectID(key._str) : key;                          // 94
                                                                                                               // 95
          collection.remove(objectId, function (error) {                                                       // 96
            if (error) {                                                                                       // 97
              deferred.reject(error);                                                                          // 98
            } else {                                                                                           // 99
              deferred.resolve({_id: objectId, action: "removed"});                                            // 100
            }                                                                                                  // 101
          });                                                                                                  // 102
        } else {                                                                                               // 103
          deferred.reject("key cannot be null");                                                               // 104
        }                                                                                                      // 105
                                                                                                               // 106
        return deferred.promise;                                                                               // 107
      }                                                                                                        // 108
                                                                                                               // 109
      /*                                                                                                       // 110
       * What to remove from collection depending on the 'keys' argument passed.                               // 111
       */                                                                                                      // 112
      if (keys) { // Checks if a 'keys' argument was passed.                                                   // 113
        if (angular.isArray(keys)) { // If an array of keys were passed.                                       // 114
          angular.forEach(keys, function (key) {                                                               // 115
            this.push(removeObject(key, $q));                                                                  // 116
          }, promises);                                                                                        // 117
        } else { // If a single key was passed.                                                                // 118
          promises.push(removeObject(keys, $q));                                                               // 119
        }                                                                                                      // 120
      } else { // If no 'keys' argument was passed, save the entire collection.                                // 121
        // When removing all, we do not use collection.remove({}) because Meteor doesn't give the client side that permissions
        // http://stackoverflow.com/a/15465286/1426570                                                         // 123
        var originalSelf = angular.copy(self);                                                                 // 124
        angular.forEach(originalSelf, function (doc) {                                                         // 125
          this.push(removeObject(doc._id, $q));                                                                // 126
        }, promises);                                                                                          // 127
      }                                                                                                        // 128
                                                                                                               // 129
      return $q.all(promises); // Returns all promises when they're resolved.                                  // 130
                                                                                                               ...
                                                                                                               // 132
    AngularMeteorCollection.updateCursor = function (cursor) {                                                 // 133
      var self = this;                                                                                         // 134
                                                                                                               // 135
      function safeApply() {                                                                                   // 136
        // Clearing the watch is needed so no updates are sent to server                                       // 137
        // while handling updates from the server                                                              // 138
        self.UPDATING_FROM_SERVER = true;                                                                      // 139
        if (!$rootScope.$$phase) $rootScope.$apply();                                                          // 140
        // Making sure we are setting to false only after one digest cycle and not before                      // 141
        $timeout(function(){                                                                                   // 142
          self.UPDATING_FROM_SERVER = false;                                                                   // 143
        },0,false);                                                                                            // 144
      }                                                                                                        // 145
                                                                                                               // 146
      // XXX - consider adding an option for a non-orderd result                                               // 147
      // for faster performance                                                                                // 148
      if (self.observeHandle) {                                                                                // 149
        self.observeHandle.stop();                                                                             // 150
      }                                                                                                        // 151
                                                                                                               // 152
      self.observeHandle = cursor.observe({                                                                    // 153
        addedAt: function (document, atIndex) {                                                                // 154
          self.splice(atIndex, 0, document);                                                                   // 155
          safeApply();                                                                                         // 156
        },                                                                                                     // 157
        changedAt: function (document, oldDocument, atIndex) {                                                 // 158
          self.splice(atIndex, 1, document);                                                                   // 159
          safeApply();                                                                                         // 160
        },                                                                                                     // 161
        movedTo: function (document, fromIndex, toIndex) {                                                     // 162
          self.splice(fromIndex, 1);                                                                           // 163
          self.splice(toIndex, 0, document);                                                                   // 164
          safeApply();                                                                                         // 165
        },                                                                                                     // 166
        removedAt: function (oldDocument) {                                                                    // 167
          var removedObject;                                                                                   // 168
          if (oldDocument._id._str){                                                                           // 169
            removedObject = _.find(self, function(obj) {                                                       // 170
              return obj._id._str == oldDocument._id._str;                                                     // 171
            });                                                                                                // 172
          }                                                                                                    // 173
          else                                                                                                 // 174
            removedObject = _.findWhere(self, {_id: oldDocument._id});                                         // 175
                                                                                                               // 176
          if (removedObject){                                                                                  // 177
            self.splice(self.indexOf(removedObject), 1);                                                       // 178
            safeApply();                                                                                       // 179
          }                                                                                                    // 180
        }                                                                                                      // 181
      });                                                                                                      // 182
                                                                                                               ...
                                                                                                               // 184
    AngularMeteorCollection.stop = function () {                                                               // 185
      if (this.unregisterAutoBind)                                                                             // 186
        this.unregisterAutoBind();                                                                             // 187
                                                                                                               // 188
      if (this.observeHandle)                                                                                  // 189
        this.observeHandle.stop();                                                                             // 190
                                                                                                               // 191
      while (this.length > 0) {                                                                                // 192
        this.pop();                                                                                            // 193
      }                                                                                                        // 194
    };                                                                                                         // 195
                                                                                                               // 196
    var createAngularMeteorCollection = function (cursor, collection) {                                        // 197
      var data = [];                                                                                           // 198
                                                                                                               // 199
      data.$$collection = angular.isDefined(collection) ? collection : $meteorUtils.getCollectionByName(cursor.collection.name);
                                                                                                               // 201
      angular.extend(data, AngularMeteorCollection);                                                           // 202
                                                                                                               // 203
      return data;                                                                                             // 204
    };                                                                                                         // 205
                                                                                                               // 206
    return createAngularMeteorCollection;                                                                      // 207
}]);                                                                                                           // 208
                                                                                                               // 209
                                                                                                               // 210
angularMeteorCollections.factory('$meteorCollection', ['AngularMeteorCollection', '$rootScope', 'diffArray',   // 211
  function (AngularMeteorCollection, $rootScope, diffArray) {                                                  // 212
    return function (reactiveFunc, auto, collection) {                                                         // 213
      // Validate parameters                                                                                   // 214
      if (!reactiveFunc) {                                                                                     // 215
        throw new TypeError("The first argument of $meteorCollection is undefined.");                          // 216
      }                                                                                                        // 217
      if (!(typeof reactiveFunc == "function" || angular.isFunction(reactiveFunc.find))) {                     // 218
        throw new TypeError("The first argument of $meteorCollection must be a function or a have a find function property.");
      }                                                                                                        // 220
      auto = auto !== false;                                                                                   // 221
                                                                                                               // 222
      if (!(typeof reactiveFunc == "function")) {                                                              // 223
        var cursorFunc = reactiveFunc.find;                                                                    // 224
        collection = angular.isDefined(collection) ? collection : reactiveFunc;                                // 225
        var originalCollection = reactiveFunc;                                                                 // 226
        reactiveFunc = function() {                                                                            // 227
          return cursorFunc.apply(originalCollection, [{}]);                                                   // 228
        }                                                                                                      // 229
      }                                                                                                        // 230
                                                                                                               // 231
      var ngCollection = new AngularMeteorCollection(reactiveFunc(), collection);                              // 232
      var realOldItems;                                                                                        // 233
                                                                                                               // 234
      function setAutoBind() {                                                                                 // 235
        if (auto) { // Deep watches the model and performs autobind.                                           // 236
          ngCollection.unregisterAutoBind = $rootScope.$watch(function () {                                    // 237
            if (ngCollection.UPDATING_FROM_SERVER){                                                            // 238
              realOldItems = _.without(ngCollection, 'UPDATING_FROM_SERVER');                                  // 239
              return 'UPDATING_FROM_SERVER';                                                                   // 240
            }                                                                                                  // 241
            return _.without(ngCollection, 'UPDATING_FROM_SERVER');                                            // 242
          }, function (newItems, oldItems) {                                                                   // 243
            if (newItems == 'UPDATING_FROM_SERVER')                                                            // 244
              return;                                                                                          // 245
                                                                                                               // 246
            if (oldItems == 'UPDATING_FROM_SERVER')                                                            // 247
              oldItems = realOldItems;                                                                         // 248
                                                                                                               // 249
                                                                                                               // 250
            if (newItems !== oldItems) {                                                                       // 251
                                                                                                               // 252
              diffArray(oldItems, newItems, {                                                                  // 253
                addedAt: function (id, item, index) {                                                          // 254
                  ngCollection.unregisterAutoBind();                                                           // 255
                  var newValue = ngCollection.splice( index, 1 )[0];                                           // 256
                  setAutoBind();                                                                               // 257
                  ngCollection.save(newValue);                                                                 // 258
                },                                                                                             // 259
                removedAt: function (id, item, index) {                                                        // 260
                  ngCollection.remove(id);                                                                     // 261
                },                                                                                             // 262
                changedAt: function (id, setDiff, unsetDiff, index, oldItem) {                                 // 263
                                                                                                               // 264
                  if (setDiff)                                                                                 // 265
                    ngCollection.save(setDiff);                                                                // 266
                                                                                                               // 267
                  if (unsetDiff)                                                                               // 268
                    ngCollection.save(unsetDiff, true);                                                        // 269
                },                                                                                             // 270
                movedTo: function (id, item, fromIndex, toIndex) {                                             // 271
                  // XXX do we need this?                                                                      // 272
                }                                                                                              // 273
              });                                                                                              // 274
            }                                                                                                  // 275
          }, true);                                                                                            // 276
                                                                                                               ...
                                                                                                               // 279
      /**                                                                                                      // 280
       * Fetches the latest data from Meteor and update the data variable.                                     // 281
       */                                                                                                      // 282
      Tracker.autorun(function () {                                                                            // 283
        // When the reactive func gets recomputated we need to stop any previous                               // 284
        // observeChanges                                                                                      // 285
        Tracker.onInvalidate(function () {                                                                     // 286
          ngCollection.stop();                                                                                 // 287
        });                                                                                                    // 288
        ngCollection.updateCursor(reactiveFunc());                                                             // 289
        setAutoBind();                                                                                         // 290
      });                                                                                                      // 291
                                                                                                               // 292
      return ngCollection;                                                                                     // 293
    }                                                                                                          // 294
  }]);                                                                                                         // 295
                                                                                                               // 296
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    return {                                                                                                   // 59
      ...
      link: function (scope, element, attributes) {                                                            // 62
        ...
        if (name && Template[name]) {                                                                          // 64
          ...
          Blaze.renderWithData(template, scope, element.get(0));                                               // 66
...
angularMeteorUser.service('$meteorUser', ['$rootScope', '$meteorUtils', '$q',                                  // 13
  function($rootScope, $meteorUtils, $q){                                                                      // 14
                                                                                                               // 15
    this.waitForUser = function(){                                                                             // 16
                                                                                                               // 17
      var deferred = $q.defer();                                                                               // 18
                                                                                                               // 19
      $meteorUtils.autorun($rootScope, function(){                                                             // 20
        if ( !Meteor.loggingIn() )                                                                             // 21
          deferred.resolve( Meteor.user() );                                                                   // 22
      });                                                                                                      // 23
                                                                                                               // 24
      return deferred.promise;                                                                                 // 25
    };                                                                                                         // 26
                                                                                                               // 27
    this.requireUser = function(){                                                                             // 28
                                                                                                               // 29
      var deferred = $q.defer();                                                                               // 30
                                                                                                               // 31
      $meteorUtils.autorun($rootScope, function(){                                                             // 32
        if ( !Meteor.loggingIn() ) {                                                                           // 33
          if ( Meteor.user() == null)                                                                          // 34
            deferred.reject("AUTH_REQUIRED");                                                                  // 35
          else                                                                                                 // 36
            deferred.resolve( Meteor.user() );                                                                 // 37
        }                                                                                                      // 38
      });                                                                                                      // 39
                                                                                                               // 40
      return deferred.promise;                                                                                 // 41
    };                                                                                                         // 42
                                                                                                               // 43
    this.loginWithPassword = function(user, password){                                                         // 44
                                                                                                               // 45
      var deferred = $q.defer();                                                                               // 46
                                                                                                               // 47
      Meteor.loginWithPassword(user, password, function(err){                                                  // 48
        if (err)                                                                                               // 49
          deferred.reject(err);                                                                                // 50
        else                                                                                                   // 51
          deferred.resolve();                                                                                  // 52
      });                                                                                                      // 53
                                                                                                               // 54
      return deferred.promise;                                                                                 // 55
    };                                                                                                         // 56
                                                                                                               // 57
    this.createUser = function(options){                                                                       // 58
                                                                                                               // 59
      var deferred = $q.defer();                                                                               // 60
                                                                                                               // 61
      Accounts.createUser(options, function(err){                                                              // 62
        if (err)                                                                                               // 63
      ...
      return deferred.promise;                                                                                 // 69
                                                                                                               ...
                                                                                                               // 71
    this.changePassword = function(oldPassword, newPassword){                                                  // 72
                                                                                                               // 73
      var deferred = $q.defer();                                                                               // 74
                                                                                                               // 75
      Accounts.changePassword(oldPassword, newPassword, function(err){                                         // 76
        if (err)                                                                                               // 77
      ...
      return deferred.promise;                                                                                 // 83
                                                                                                               ...
                                                                                                               // 85
    this.forgotPassword = function(options){                                                                   // 86
                                                                                                               // 87
      var deferred = $q.defer();                                                                               // 88
                                                                                                               // 89
      Accounts.forgotPassword(options, function(err){                                                          // 90
        if (err)                                                                                               // 91
      ...
      return deferred.promise;                                                                                 // 97
                                                                                                               ...
                                                                                                               // 99
    this.resetPassword = function(token, newPassword){                                                         // 100
                                                                                                               // 101
      var deferred = $q.defer();                                                                               // 102
                                                                                                               // 103
      Accounts.resetPassword(token, newPassword, function(err){                                                // 104
        if (err)                                                                                               // 105
      ...
      return deferred.promise;                                                                                 // 111
                                                                                                               ...
                                                                                                               // 113
    this.verifyEmail = function(token){                                                                        // 114
                                                                                                               // 115
      var deferred = $q.defer();                                                                               // 116
                                                                                                               // 117
      Accounts.verifyEmail(token, function(err){                                                               // 118
        if (err)                                                                                               // 119
      ...
      return deferred.promise;                                                                                 // 125
                                                                                                               ...
                                                                                                               // 127
    this.logout = function(){                                                                                  // 128
                                                                                                               // 129
      var deferred = $q.defer();                                                                               // 130
                                                                                                               // 131
      Meteor.logout(function(err){                                                                             // 132
          if (err)                                                                                             // 133
      ...
      return deferred.promise;                                                                                 // 139
                                                                                                               ...
                                                                                                               // 141
    this.logoutOtherClients = function(){                                                                      // 142
                                                                                                               // 143
      var deferred = $q.defer();                                                                               // 144
                                                                                                               // 145
      Meteor.logoutOtherClients(function(err){                                                                 // 146
        if (err)                                                                                               // 147
      ...
      return deferred.promise;                                                                                 // 153
                                                                                                               ...
                                                                                                               // 155
    this.loginWithFacebook = function(options){                                                                // 156
                                                                                                               // 157
      var deferred = $q.defer();                                                                               // 158
                                                                                                               // 159
      Meteor.loginWithFacebook(options, function(err){                                                         // 160
        if (err)                                                                                               // 161
      ...
      return deferred.promise;                                                                                 // 167
                                                                                                               ...
                                                                                                               // 169
    this.loginWithTwitter = function(options){                                                                 // 170
                                                                                                               // 171
      var deferred = $q.defer();                                                                               // 172
                                                                                                               // 173
      Meteor.loginWithTwitter(options, function(err){                                                          // 174
        if (err)                                                                                               // 175
      ...
      return deferred.promise;                                                                                 // 181
                                                                                                               ...
                                                                                                               // 183
    this.loginWithGoogle = function(options){                                                                  // 184
                                                                                                               // 185
      var deferred = $q.defer();                                                                               // 186
                                                                                                               // 187
      Meteor.loginWithGoogle(options, function(err){                                                           // 188
        if (err)                                                                                               // 189
      ...
      return deferred.promise;                                                                                 // 195
                                                                                                               ...
                                                                                                               // 197
    this.loginWithGithub = function(options){                                                                  // 198
                                                                                                               // 199
      var deferred = $q.defer();                                                                               // 200
                                                                                                               // 201
      Meteor.loginWithGithub(options, function(err){                                                           // 202
        if (err)                                                                                               // 203
      ...
      return deferred.promise;                                                                                 // 209
                                                                                                               ...
                                                                                                               // 211
    this.loginWithMeteorDeveloperAccount = function(options){                                                  // 212
                                                                                                               // 213
      var deferred = $q.defer();                                                                               // 214
                                                                                                               // 215
      Meteor.loginWithMeteorDeveloperAccount(options, function(err){                                           // 216
        if (err)                                                                                               // 217
      ...
      return deferred.promise;                                                                                 // 223
                                                                                                               ...
                                                                                                               // 225
    this.loginWithMeetup = function(options){                                                                  // 226
                                                                                                               // 227
      var deferred = $q.defer();                                                                               // 228
                                                                                                               // 229
      Meteor.loginWithMeetup(options, function(err){                                                           // 230
        if (err)                                                                                               // 231
      ...
      return deferred.promise;                                                                                 // 237
                                                                                                               ...
                                                                                                               // 239
    this.loginWithWeibo = function(options){                                                                   // 240
                                                                                                               // 241
      var deferred = $q.defer();                                                                               // 242
                                                                                                               // 243
      Meteor.loginWithWeibo(options, function(err){                                                            // 244
        if (err)                                                                                               // 245
...
angularMeteorReactiveScope.run(['$rootScope', '$parse', function($rootScope, $parse) {                         // 6
  Object.getPrototypeOf($rootScope).getReactively = function(property) {                                       // 7
    var self = this;                                                                                           // 8
    var getValue = $parse(property);                                                                           // 9
                                                                                                               // 10
    if (!self.$$trackerDeps) {                                                                                 // 11
      self.$$trackerDeps = {};                                                                                 // 12
    }                                                                                                          // 13
                                                                                                               // 14
    if (!self.$$trackerDeps[property]) {                                                                       // 15
      self.$$trackerDeps[property] = new Tracker.Dependency();                                                 // 16
                                                                                                               // 17
      self.$watch(function() {                                                                                 // 18
        return getValue(self)                                                                                  // 19
      }, function(newVal, oldVal) {                                                                            // 20
        if (newVal !== oldVal) {                                                                               // 21
          self.$$trackerDeps[property].changed();                                                              // 22
        }                                                                                                      // 23
      });                                                                                                      // 24
    }                                                                                                          // 25
                                                                                                               // 26
    self.$$trackerDeps[property].depend();                                                                     // 27
                                                                                                               // 28
    return getValue(self);                                                                                     // 29
  };                                                                                                           // 30
}]);                                                                                                           // 31
                                                                                                               // 32
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
angularMeteor.run(['$compile', '$document', '$rootScope', function ($compile, $document, $rootScope) {         // 17
    ...
    if(typeof Router != 'undefined') {                                                                         // 19
      Router.onAfterAction(function(req, res, next) {                                                          // 20
        // Since Router.current().ready() is reactive we wrap it in Tracker.nonreactive                        // 21
        // to avoid re-runs.                                                                                   // 22
        Tracker.nonreactive(function() {                                                                       // 23
          Tracker.afterFlush(function() {                                                                      // 24
            var route = Router.current();                                                                      // 25
            if (route.ready()) {                                                                               // 26
              // Since onAfterAction runs always twice when a route has waitOn's subscriptions,                // 27
              // we need to handle case when data is already loaded at the moment                              // 28
              // Tracker.afterFlush executes which means it will run twice with                                // 29
              // Router.current().ready equals true.                                                           // 30
              // That's we save state to an additional auxiliry variable _done.                                // 31
              if (!route.state.get('__rendered')) {                                                            // 32
                // Checks if document's been compiled to the moment.                                           // 33
                // If yes, compile only newly inserted parts.                                                  // 34
                if (route.state.get('__compiled')) {                                                           // 35
                  for (var prop in route._layout._regions) {                                                   // 36
                    var region = route._layout._regions[prop];                                                 // 37
                    var node = region.view._domrange.firstNode();                                              // 38
                    while (node) {                                                                             // 39
                       $compile(node)($rootScope);                                                             // 40
                       node = node.nextSibling;                                                                // 41
                    }                                                                                          // 42
                  }                                                                                            // 43
                } else {                                                                                       // 44
                  $compile($document)($rootScope);                                                             // 45
                }                                                                                              // 46
                if (!$rootScope.$$phase) $rootScope.$apply();                                                  // 47
                route.state.set('__rendered', true);                                                           // 48
              }                                                                                                // 49
            } else {                                                                                           // 50
                // Compiles and applies scope for the first time when current route is not ready.              // 51
                $compile($document)($rootScope);                                                               // 52
                if (!$rootScope.$$phase) $rootScope.$apply();                                                  // 53
                route.state.set('__compiled', true);                                                           // 54
             }                                                                                                 // 55
          });                                                                                                  // 56
        });                                                                                                    // 57
      });                                                                                                      // 58
                                                                                                               ...
                                                                                                               // 61
// Putting all services under $meteor service for syntactic sugar                                              // 62
angularMeteor.service('$meteor', ['$meteorCollection', '$meteorObject', '$meteorMethods', '$meteorSession', '$meteorSubscribe', '$meteorUtils', '$meteorCamera', '$meteorUser',
  function($meteorCollection, $meteorObject, $meteorMethods, $meteorSession, $meteorSubscribe, $meteorUtils, $meteorCamera, $meteorUser){
    this.collection = $meteorCollection;                                                                       // 65
    this.object = $meteorObject;                                                                               // 66
    this.subscribe = $meteorSubscribe.subscribe;                                                               // 67
    this.call = $meteorMethods.call;                                                                           // 68
    this.loginWithPassword = $meteorUser.loginWithPassword;                                                    // 69
    this.requireUser = $meteorUser.requireUser;                                                                // 70
    this.waitForUser = $meteorUser.waitForUser;                                                                // 71
    this.createUser = $meteorUser.createUser;                                                                  // 72
    this.changePassword = $meteorUser.changePassword;                                                          // 73
    this.forgotPassword = $meteorUser.forgotPassword;                                                          // 74
    this.resetPassword = $meteorUser.resetPassword;                                                            // 75
    this.verifyEmail = $meteorUser.verifyEmail;                                                                // 76
    this.loginWithMeteorDeveloperAccount = $meteorUser.loginWithMeteorDeveloperAccount;                        // 77
    this.loginWithFacebook = $meteorUser.loginWithFacebook;                                                    // 78
    this.loginWithGithub = $meteorUser.loginWithGithub;                                                        // 79
    this.loginWithGoogle = $meteorUser.loginWithGoogle;                                                        // 80
    this.loginWithMeetup = $meteorUser.loginWithMeetup;                                                        // 81
    this.loginWithTwitter = $meteorUser.loginWithTwitter;                                                      // 82
    this.loginWithWeibo = $meteorUser.loginWithWeibo;                                                          // 83
    this.logout = $meteorUser.logout;                                                                          // 84
    this.logoutOtherClients = $meteorUser.logoutOtherClients;                                                  // 85
    this.session = $meteorSession;                                                                             // 86
    this.autorun = $meteorUtils.autorun;                                                                       // 87
    this.getCollectionByName = $meteorUtils.getCollectionByName;                                               // 88
    this.getPicture = $meteorCamera.getPicture;                                                                // 89
}]);                                                                                                           // 90
                                                                                                               // 91
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
