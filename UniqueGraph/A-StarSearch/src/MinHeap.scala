...
class MinHeap {
  ...
  def MinHeapify(A: ArrayBuffer[GraphNode], i: Int) {
    ...
    if ((left <= A.length - 1) && (A(left).getFunction() < A(min).getFunction())) {
      min = left;
    }

    if ((right <= A.length - 1) && (A(right).getFunction() < A(min).getFunction())) {
      min = right;
    }
  ...
  def siftup(A: ArrayBuffer[GraphNode], nodeIndex: Int) {
    if (nodeIndex != 0) {
      ...
      if (A(nodeIndex).getFunction() < A(parentNode).getFunction()) {
